...
# 1. Load Data
file_path = 'NGO_Project_MNE_Dataset_2000.xlsx'
...
# 3. Setup Plot
plt.style.use('default')
fig, ax = plt.subplots(figsize=(12, 6))  # Wider for better label spacing
fig.patch.set_facecolor('#ffffff')
ax.set_facecolor('#f8fafc')

# 4. Create Chart - use colormap for dynamic color handling
colors = plt.cm.tab20(range(len(district_counts)))
district_counts.plot(kind='bar', ax=ax, color=colors)
...
ax.tick_params(axis='x', rotation=45, colors='#1e293b')  # Fixed rotation
ax.tick_params(axis='y', colors='#1e293b')
ax.grid(axis='y', linestyle='--', alpha=0.5, color='#94a3b8')
...
plt.tight_layout()
plt.savefig('district_chart.png', dpi=150)
print("Chart saved as district_chart.png")
